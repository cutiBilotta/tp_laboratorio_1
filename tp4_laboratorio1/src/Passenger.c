...
int Passenger_setNombre(Passenger* p, char* nombre)
{
    ...
    if(p != NULL && nombre != NULL && (strlen(nombre) <50 && strlen(nombre) > 0))
    {
...
int Passenger_setCodigoVuelo(Passenger* p,char* codigoVuelo)
{
    ...
    if(p != NULL && codigoVuelo != NULL )
    {
        ...
        if(valido && strlen(auxStr)>0 && strlen(auxStr)<10)
        {
            strupr(auxStr);
...
int Passenger_setTipoPasajero(Passenger* p,char* tipoPasajero)
{
    ...
    if(p != NULL && tipoPasajero != NULL && (strlen(tipoPasajero) <=50 && strlen(tipoPasajero) > 0))
    {
        strcpy(auxStr, tipoPasajero);
        for(int i=0; i<strlen(auxStr); i++)
        {
            if(!(auxStr[i]>=65 && auxStr[i]<=90) && !(auxStr[i]>=97 && auxStr[i]<=122))
            {
                error=-1;
                valido=0;
                break;
...
int Passenger_getEstadoVuelo(Passenger* p,char* estadoVuelo)
{
    ...
    if(p != NULL && estadoVuelo != NULL && (strlen(estadoVuelo) <=50 && strlen(estadoVuelo) > 0))
    {
